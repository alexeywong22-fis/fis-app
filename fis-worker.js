...
- 門檻（明顯優先）：對應特徵為「明顯」且有兩個相片來源確認 → 該線優先關注(🔴)；僅「輕微」／單一來源／存疑 → 狀態良好(🟢)。
- 未評估（Stage 2）項目：以可得嘅靜態特徵判定該線，唔好因動態缺席就當🟢，亦唔好硬當🔴。
- 對照規則（讀靜態清單）：
  · 圓肩／頭部前移達門檻 → 深前線、淺前線 🔴
  · 骨盆前傾／腰椎弧度偏大達門檻 → 深前線、淺背線 🔴
  · 肩高不對稱／肩胛骨不對稱達門檻 → 螺旋線 🔴
  · 足弓特徵／膝靜態對齊異常達門檻 → 側線 🔴
  · 對應靜態項目全部「無」或未達門檻 → 該線 🟢
- 對應到 JSON：🔴 → status「優先關注」、stage「Stage 1」；🟢 → status「狀態良好」、stage「Stage 2」。
- 90/90 版本：①骨盆完全對稱前傾且無股骨內旋／胸椎代償／骨盆旋轉／側傾 → 標準版；②骨盆中立 → 修改版；③骨盆完全對稱後傾且無上述 → 跳過；④任何骨盆位置只要有股骨內旋／胸椎代償／骨盆旋轉／側傾任一 → 修改版；⑤任何疑問或不清晰（含因動態未評估）→ 修改版。
- 訓練計劃：任何一條 🔴 → 「FIS鋼鐵解鎖(9.3A)」；全部 🟢 → 「FIS奇才通關(9.3B)」。

只輸出以下 JSON（status 只能「優先關注」或「狀態良好」；stage 只能「Stage 1」或「Stage 2」）：
{
  "fascialLines": {
  ...
  "recommendations": {
    ...
    "startingPoint": "相片版評估，動態項目（步態／單腳／呼吸）留待影片版補充；[有🔴→由優先線 Stage 1 起／全🟢→可由 Stage 2 起]"
  }
}
只輸出 JSON。`
}];
